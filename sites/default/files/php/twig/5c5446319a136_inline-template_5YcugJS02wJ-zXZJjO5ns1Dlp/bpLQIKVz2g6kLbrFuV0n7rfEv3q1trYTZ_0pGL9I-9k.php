...
class __TwigTemplate_c49bd5ef621a21b509bb7e01331729d64d69e20c76562dd003290542cdb73e8b extends Twig_Template
{
    ...
    protected function doDisplay(array $context, array $blocks = [])
    {
        ...
        if ((($context["description"] ?? null) || ($context["warning"] ?? null))) {
            ...
            if (($context["warning"] ?? null)) {
                ...
                echo $this->env->getExtension('Twig_Extension_Sandbox')->ensureToStringAllowed($this->env->getExtension('Drupal\Core\Template\TwigExtension')->escapeFilter($this->env, ($context["warning"] ?? null), "html", null, true));
                echo "</em> ";
            }
    ...
    {
        return new Twig_Source("{# inline_template_start #}<div class=\"permission\"><span class=\"title\">{{ title }}</span>{% if description or warning %}<div class=\"description\">{% if warning %}<em class=\"permission-warning\">{{ warning }}</em> {% endif %}{{ description }}</div>{% endif %}</div>", "{# inline_template_start #}<div class=\"permission\"><span class=\"title\">{{ title }}</span>{% if description or warning %}<div class=\"description\">{% if warning %}<em class=\"permission-warning\">{{ warning }}</em> {% endif %}{{ description }}</div>{% endif %}</div>", "");
    }
}
